...
@onready var speed_1x: Button = %Speed1x
@onready var speed_2x: Button = %Speed2x
@onready var speed_3x: Button = %Speed3x
...
func _connect_signals() -> void:
	...
	speed_1x.pressed.connect(func() -> void: _set_speed(1.2, speed_1x))
	speed_2x.pressed.connect(func() -> void: _set_speed(0.75, speed_2x))
	speed_3x.pressed.connect(func() -> void: _set_speed(0.45, speed_3x))
	%GameOverToTitleButton.pressed.connect(func() -> void: get_tree().change_scene_to_file("res://scenes/TitleScreen.tscn"))
...
func _refresh_top_bar() -> void:
	money_value.text = "%s원" % GameText.format_number(game_state.money)
...
func _render_recent_logs() -> void:
	...
	for entry: Dictionary in _recent_logs:
		...
		item.theme_type_variation = "LogLabel"
...
func _set_speed(interval: float, active_button: Button) -> void:
	round_manager.tick_interval_sec = interval
	for b: Button in [speed_1x, speed_2x, speed_3x]:
		b.disabled = b == active_button
