...
func update_demands(demands: Array[int]) -> void:
	for i: int in min(demands.size(), floor_demand_labels.size()):
		...
		floor_demand_labels[i].modulate = Color("#FFD28A") if warning else Color("#D3E4FF")
		floor_bars[i].size.x = clampf(20 + value * 6.0, 20, 160)
		floor_bars[i].color = Color("#F28B52") if value >= 10 else (Color("#E7C35A") if warning else Color("#56C7FF"))
		floor_dots[i].text = "■■■" if value >= 10 else ("■■□" if warning else "■□□")
...
func set_selected_elevator(elevator_id: int) -> void:
	...
	marker_a.add_theme_color_override("font_color", Color("#9DC7FF"))
	marker_b.add_theme_color_override("font_color", Color("#9DC7FF"))
	if elevator_id == 1:
...
func _update_single(car: PanelContainer, label: Label, marker: Label, elevator: ElevatorData, color_resolver: Callable) -> void:
	...
	create_tween().tween_property(car, "position:y", target_y, 0.25).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	label.text = "%s  %.0f%%" % [elevator.name, elevator.load * 100.0]
	label.modulate = Color("#F5FBFF")
	...
	if elevator.status == "fault":
		...
		flash.tween_property(car, "modulate", Color("#FF5A5A"), 0.1)
		flash.tween_property(car, "modulate", color_resolver.call(elevator.status), 0.14)

func _marker_text(status: String) -> String:
	match status:
		"fault":
			return "!!!"
		"warning", "inspection_due", "risk":
			return "!"
		"busy":
			return "<>"
		_:
